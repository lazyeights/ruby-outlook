...
module Pst
  ...
  class CompressibleEncryption
    ...
    ENCRYPT_TABLE = Array.new(256)
    DECRYPT_TABLE.each_with_index { | key, index | ENCRYPT_TABLE[key] = index }

    DECRYPT_KEY_PAD, ENCRYPT_KEY_PAD = [DECRYPT_TABLE, (0...256)].map do | keys |
      keys.map { | key | key.chr }.join.gsub(/([\^\-\\])/, "\\\\\\1")
    end

    def self.decrypt ciphertext
      ciphertext.tr ENCRYPT_KEY_PAD, DECRYPT_KEY_PAD
    end

    def self.encrypt cleartext
      cleartext.tr DECRYPT_KEY_PAD, ENCRYPT_KEY_PAD
    end
    
  end

  class HighEncryption
    # TODO
  end
